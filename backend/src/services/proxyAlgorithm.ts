...
export async function getAvailableTeachers(
    ...
    for (const teacher of allTeachers) {
        ...
        // Check subject match
        const teachingSubjects = teacher.teachingSubjects as any;
        ...
        if (previousPeriodId) {
            ...
            if (!teachingPrevious) adjacentFree = true;
        }
        if (nextPeriodId && !adjacentFree) {
            const teachingNext = teacher.timetables.some(t => t.periodId === nextPeriodId);
            if (!teachingNext) adjacentFree = true;
        }

        // Calculate score (lower is better)
        let score = totalLoad; // Base score is the load
        if (subjectMatch) score -= 2; // Bonus for subject match
        if (adjacentFree) score -= 1; // Bonus for adjacent free period
