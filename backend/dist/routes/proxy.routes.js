...
const DAY_TO_ENUM = {
    ...
    1: client_1.DayOfWeek.TUESDAY,
    2: client_1.DayOfWeek.WEDNESDAY,
    3: client_1.DayOfWeek.THURSDAY,
    4: client_1.DayOfWeek.FRIDAY,
    5: 'SATURDAY'
};
...
router.get('/available-teachers', async (req, res) => {
    try {
        ...
        if (excludeTeacherId) {
            unavailableIds.push(excludeTeacherId);
        }
        ...
        const teachersWithLoad = await Promise.all(availableTeachers.map(async (teacher) => {
...
router.post('/assignments/bulk', async (req, res) => {
    try {
        ...
        const createdProxies = await Promise.all(assignments.map(async (assignment) => {
...
router.get('/assignments', async (req, res) => {
    try {
        ...
        const selectedDate = new Date(date);
        const where = { date: selectedDate };
        if (teacherId) {
            where.absentTeacherId = teacherId;
        }
...
router.get('/assignments/date/:date', async (req, res) => {
    try {
